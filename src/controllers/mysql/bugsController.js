...
// import { getDb } from '../../models/mysql/db.js'
...
//     // total count header (useful for frontends)
...
//   try {
//     const db = getDb()
//     const [rows] = await db.query('SELECT COUNT(*) AS total FROM tasks')
...
//       id: insertId, title, description, priority, status,
...
//   try {
//     const id = toNumId(req.params.id); if (id === null) return res.status(400).json({ error: 'Invalid ID' })
//     const fields = { ...req.body }; delete fields.id
//     const keys = Object.keys(fields); if (keys.length === 0) return res.status(400).json({ error: 'No fields provided' })

//     const setClause = keys.map(k => `\`${k}\` = ?`).join(', ')
//     const values = keys.map(k => fields[k]); values.push(id)

//     const db = getDb()
//     const [result] = await db.execute(`UPDATE tasks SET ${setClause}, updated_at = NOW() WHERE id = ?`, values)
//     if (result.affectedRows === 0) return res.status(404).json({ error: 'Task not found' })

//     const [rows] = await db.execute(`SELECT id, title, description, status, priority, created_at, updated_at FROM tasks WHERE id = ?`, [id])
//     const updated = rows[0]
//     const io = req.app.get('io'); if (io) io.emit('taskUpdated', updated)
//     res.json(updated)
//   } catch (err) {
...
//     const io = req.app.get('io'); if (io) io.emit('taskDeleted', id)
  ...
  try {
    ...
    try {
      ...
      const [findRows] = await db.execute(
        `SELECT id, title, description, status, priority, created_at, updated_at
         FROM tasks
         WHERE title = ? AND description = ? AND priority = ? AND status = ?
         ORDER BY created_at DESC
         LIMIT 1`,
        [title, description, priority, status]
      )
...
export async function patchTask(req, res) {
  try {
    ...
    const priority = 'priority' in fields ? fields.priority : null
    const status = 'status' in fields ? fields.status : null

    try {
      // Try stored procedure first; the procedure should update only non-null fields
      await db.execute('CALL sp_patch_task(?, ?, ?, ?, ?)', [
        id,
        title,
        description,
        priority,
...
export async function deleteTask(req, res) {
  try {
    ...
    try {
      ...
      const [rows] = await db.execute('SELECT 1 FROM tasks WHERE id = ? LIMIT 1', [id])
      if (rows.length > 0) {
        // still exists
