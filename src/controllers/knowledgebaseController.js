...
//       }
//       const io = req.app.get('io')
//       if (io) io.emit('kbCreated', newTask)
//       return res.status(201).json(newTask)
//     } catch (procErr) {
...
export async function patchKB(req, res) {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ error: 'KB entry not found' })
    ...
    console.error('Error patching KB entry:', err)
    res.status(500).json({ error: 'Database error while updating KB entry' })
  }
}
